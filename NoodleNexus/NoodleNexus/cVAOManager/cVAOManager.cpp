	...
	unsigned int shadowShaderProgramID)
{
		...
		itFN != vecFileNames.end(); itFN++)
	{
		...
		itFN->second = this->LoadModelIntoVAO(fileName, basepath, meshDrawInfo, shaderProgramID, shadowShaderProgramID);
	...
	unsigned int shadowShaderProgramID)
{
	...
	this->m_CopyLoadedModelToVAO(drawInfo, shadowShaderProgramID);
